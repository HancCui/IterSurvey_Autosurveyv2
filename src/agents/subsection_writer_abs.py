...
from src.prompt import SECTION_WO_SUBSECTION_WRITING_PROMPT_STRUCTURED, SECTION_W_SUBSECTION_SUMMARIZING_PROMPT_STRUCTURED, PAPER_CARD_PROMPT
...
class subsectionWriter():
    ...
    def __init__(self, model, api_key, api_url, database, max_len = 11000, use_abs=True, input_graph = False, vision_model = None, vision_api_key=None, vision_api_url=None) -> None:
        ...
        print("Using `subsection_writer_abs`.")
        ...
        for _ in range(3):
            try:
                if not section.subsections:
                    ...
                    response_section_content = self.api_model.chat(section_writing_prompt, temperature=0.6, check_cache=True, schema=SECTION_WO_SUBSECTION_WRITING_schema) # type: ignore
                    ...
                    new_section = Section(
                        title = response_section_content.title,
                        description = section.description,
                        content = response_section_content.content,
                    ...
                    for sub_sec_idx, sub_sec in enumerate(section.subsections):
                        ...
                        subsection_content = self.api_model.chat(section_writing_prompt, temperature=0.6, check_cache=True, schema=SECTION_WO_SUBSECTION_WRITING_schema) # type: ignore
                        ...
                        subsection_content_list.append(subsection_content.content)
                    ...
                    # 2.2 call the model
                    response_section_summary = self.api_model.chat(section_summary_prompt, temperature=0.6, check_cache=True, schema=SECTION_W_SUBSECTION_SUMMARIZING_schema) # type: ignore
                    ...
                    new_section = Section(
                        title = response_section_summary.title,
                        description = section.description,
                        content = response_section_summary.summary,
...
def parse_args():
    ...
    parser.add_argument('--use_abs',default=True, type=bool, help='Whether to use abstract or paper content for auto-survey. If true, the max_len would be set to 1500 by default')
