...
import torch
from src_legend.model import APIModel
import time
from src_legend.utils import tokenCounter
import copy
import json
from src_legend.database import database
from src_legend.prompt import SUBSECTION_WRITING_PROMPT, LCE_PROMPT, CHECK_CITATION_PROMPT
from transformers import AutoModel, AutoTokenizer,  AutoModelForSequenceClassification
from src_legend.utils import collate_text_with_imgs

class subsectionWriter():

    def __init__(self, model:str, api_key:str, api_url:str,  database, use_abs = True, max_len = 1500, input_graph = False) -> None:

        self.model, self.api_key, self.api_url, self.use_abs, self.max_len, self.input_graph = model, api_key, api_url, use_abs, max_len, input_graph
    ...
    def write(self, topic, outline, rag_num = 30, subsection_len = 500, refining = True, reflection=True):
        ...
        temp_image_dic = {}
        if not self.use_abs:
            references_papers = self.db.get_paper_from_ids(list(set(total_ids)), max_len = self.max_len)
            temp_content_dic = {id:p['text'] for id, p in zip(list(set(total_ids)), references_papers)}
            if self.input_graph:
                temp_image_dic = {k: v for p in references_papers for k, v in p['imgs'].items()}
        else:
            temp_content_dic = {p['id']:p['abs'] for p in total_references_infos}
        ...
        for i in range(len(parsed_outline['sections'])):
            for references_ids in section_references_ids[i]:
                ...
                references_papers = [temp_content_dic[_] for _ in references_ids]
                paper_texts = ''
        ...
        # Just for Debugging
        for i in range(len(parsed_outline['sections'])):
            section_content[i] = self.write_subsection_with_reflection(
    ...
    def write_subsection_with_reflection(self, paper_texts_l, topic, outline, section, subsections, subdescriptions, res_l, idx, image_dict = None, rag_num = 20, subsection_len = 1000, citation_num = 8):
        ...
        for j in range(len(subsections)):
            ...
            prompt = self.__generate_prompt(
                ...
                paras=prompt_generation_paras
            )

            if self.input_graph:
                prompt = collate_text_with_imgs(prompt, image_dict)
        ...
        res_l[idx] = contents
        print(f"Finished writing subsection {idx+1} with reflection.")
        print(f"Current usage: {self.get_writer_usage()}")
        return contents

    def __generate_prompt(self, template, paras, images = None):
        prompt = template
        for k in paras.keys():
            prompt = prompt.replace(f'[{k}]', paras[k])
        # if images is not None: # Actually we don't need it?
        #     prompt = collate_text_with_imgs(prompt, images)
        return prompt
    ...
    def lce(self, topic, outline, contents, res_l, idx):
        ...
        '''
        # 初始化为空字符串
        previous = subsection = following = ''

        # 根据contents长度分配值
        if len(contents) >= 1:
            subsection = contents[0] if len(contents) == 1 else contents[1]
        ...
        prompt = self.__generate_prompt(LCE_PROMPT, paras={'OVERALL OUTLINE': outline,'PREVIOUS': previous,\
                                                                          'FOLLOWING':following,'TOPIC':topic,'SUBSECTION':subsection})
